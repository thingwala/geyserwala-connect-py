...
class Server:
    ...
    def _unauthorised(self):
        return web.json_response(
            data={"success": False, "message": "Unauthorized"}, status=401
        )
...
class Display:
    ...
    def _refresh(self):
        ...
        self._stdscr.addstr(
            0,
            0,
            f"Geyserwala Mock [{self._gw._bind}:{self._gw._port}]",
            curses.A_REVERSE,
        )
        ...
        self._stdscr.addstr(
            8, 0, t("(q,a) Setpoint:      " + str(self._gw.value["setpoint"]))
        )
        self._stdscr.addstr(
            9, 0, t("(w,s) Water Temp:     " + str(self._gw.value["tank-temp"]))
        )
        self._stdscr.addstr(
            10, 0, t("(e,d) Collector Temp: " + str(self._gw.value["collector-temp"]))
        )
        self._stdscr.addstr(
            11,
            0,
            t(
                "(p) Pump:    "
                + ("RUNNING" if self._gw.value["pump-status"] else "STOPPED")
            ),
        )
        self._stdscr.addstr(
            12,
            0,
            t("(h) Element: " + ("ON" if self._gw.value["element-demand"] else "OFF")),
        )
        self._stdscr.addstr(
            13,
            0,
            t("(b) Boost:   " + ("ON" if self._gw.value["boost-demand"] else "OFF")),
        )
        self._stdscr.addstr(
            15, 0, t(f"(r) Register mDNS ({self._gw.value['hostname']})")
        )
