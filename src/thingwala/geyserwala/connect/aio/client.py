...
class GeyserwalaClientAsync:
    ...
    async def update_status(self):
        now = self._now()
        if (self._last_status_update + self._cache_time) > now:
            return True

        keys = list(self._status_keys)
        keys.extend(self._info_keys)
    ...
    @property
    def tank_temp(self):
        return self._values.get("tank-temp", 0)

    @property
    def collector_temp(self):
        return self._values.get("collector-temp", 0)
